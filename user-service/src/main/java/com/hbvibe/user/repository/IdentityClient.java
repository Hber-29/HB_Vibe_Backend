...
import com.hbvibe.user.dto.identity.TokenExchangeResponse;
...
public interface IdentityClient {
    @PostMapping(value= "/realms/HBvibe_Web/protocol/openid-connect/token"
            ,consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE)
    TokenExchangeResponse exchangeToken(@QueryMap TokenExchangeParam param);

    @PostMapping(value="/admin/realms/HBvibe_Web/users"
            ,consumes = MediaType.APPLICATION_JSON_VALUE)
    ResponseEntity<?> createUser(@RequestHeader("authorization") String token
            , @RequestBody UserCreationParam param);

    @DeleteMapping(value = "/admin/realms/HBvibe_Web/users/{id}")
